...
export interface Character {
  ...
  attributes: {
    skill: number
    stamina: { current: number; maximum: number; temporary: number }
    luck: { current: number; maximum: number; timesTestedThisSession: number }
  }
...
export interface Background {
  ...
  name: string
  description?: string
  advancedSkills?: Array<{ name: string; rank: number }>
...
export interface Manifest {
  metadata: { attribution?: string; title?: string }
  rules: Record<string, any>
  backgrounds: Array<Background | { $ref: string }>
  skills?: Array<any>
  spells?: Array<any>
  tables?: Array<any>
}
